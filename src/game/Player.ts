...
export class Player {
  ...
  private backgroundType: 'home' | 'projects' = 'home';
  private x: number = 0;
  private y: number = 0;
  ...
  private currentAnimation: 'idle' | 'walk' | 'jump' = 'idle';
  private animationFrame: number = 0;
  private animationTimer: number = 0;
  private groundLevel: number = 0; // The horizontal plane the player stands on (y position)
  
  // Physics constants
  private readonly EARTH_GRAVITY: number = 0.8;
  private readonly LUNAR_GRAVITY: number = 0.8 / 6; // Lunar gravity is ~1/6th of Earth's (~0.133)
  private readonly EARTH_JUMP_STRENGTH: number = 18; // Jump strength on Earth
  ...
  private readonly LUNAR_JUMP_STRENGTH: number = 12; // Slower than Earth (12 vs 18) but higher peak
  private readonly EARTH_WALK_SPEED: number = 5;
  private readonly LUNAR_WALK_SPEED: number = 5 * 0.5; // Lunar walk is slower (~2.5)
  private readonly VERTICAL_SPEED: number = 4; // Speed for up/down arrow movement
  private jumpPressed: boolean = false; // Track if jump was just pressed
  private GROUND_Y: number; // Bottom of viewport (calculated)
  ...
  private getJumpStrength(): number {
  ...
  private readonly IDLE_FRAMES: number = 4;
  private readonly WALK_FRAMES: number = 4;
  private readonly JUMP_FRAMES: number = 2;

  constructor(container: HTMLElement, backgroundType: 'home' | 'projects' = 'home') {
  ...
  private calculateGroundY(backgroundType: 'home' | 'projects'): void {
  ...
  public setContainer(newContainer: HTMLElement, backgroundType: 'home' | 'projects' = 'home'): void {
  ...
  public update(keys: { left: boolean; right: boolean; up: boolean; down: boolean; space: boolean }): void {
    const containerWidth = this.container.offsetWidth || window.innerWidth;
    const containerHeight = this.container.offsetHeight || (window.innerHeight * 0.6); // Container is 60vh
  ...
  private setAnimation(animation: 'idle' | 'walk' | 'jump'): void {
  ...
  private updateSpriteVisual(): void {
  ...
  private getColorForAnimation(anim: string, frame: number): string {
