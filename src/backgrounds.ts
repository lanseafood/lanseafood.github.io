...
class StartBackground {
  ...
  private cloud4: HTMLElement | null = null;
  private cloud5: HTMLElement | null = null;
  ...
  private createCloud(initialX: number, top: string, width: string, height: string, speed: number, opacity: number, resetX: number = -200): HTMLElement {
    ...
    (cloud as any).resetX = resetX; // Position to reset to when looping
  ...
  private animate(): void {
    const animateCloud = (cloud: HTMLElement) => {
      ...
      const resetX = (cloud as any).resetX;
    ...
    const loop = () => {
      ...
      if (this.cloud4) animateCloud(this.cloud4);
      if (this.cloud5) animateCloud(this.cloud5);
